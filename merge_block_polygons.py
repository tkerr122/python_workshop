...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "merge_polygons.log"
            ),
            mode="w",
        )
    ],
...
def get_block_envelope(block_info: BlockInfo, raster_info: RasterInfo) -> tuple:
    ...
    )  # top edge since pixel_height is negative
...
def touches_block_edge(geom_env: tuple, block_env: tuple, tol: float) -> bool:
    ...
    return (
        abs(gminx - bminx) < tol
        or abs(gminy - bminy) < tol
        or abs(gmaxx - bmaxx) < tol
        or abs(gmaxy - bmaxy) < tol
    )
...
def get_window_candidates(cand_layer, window_env: tuple) -> list:
    ...
    for feat in cand_layer:
        ...
        features.append((feat.GetFID(), geom.Clone(), feat.GetField("block_id"), env))
...
# =============================================================================
def merge_vectors(
    input_dir, output_path, raster_info: RasterInfo, blocksize, buffer_dist, progress
):
    # ---------------------------------
    # PASS 1: get touching polygons
    # ---------------------------------
    task1 = progress.add_task(
        "Finding touching polygons...", total=len(os.listdir(input_dir))
    )
    ...
    cand_layer = cand_ds.CreateLayer(
        "candidates", srs=raster_info.projection, geom_type=ogr.wkbPolygon
    )
    ...
    task2 = progress.add_task(
        "Merging touching polygons by window...", total=total_windows
    )
    ...
    for row_start in range(0, n_blocks_y, stride):
        for col_start in range(0, n_blocks_x, stride):
            ...
            win_xmin = (
                raster_info.xmin + col_start * blocksize * raster_info.pixel_width
            )
            win_xmax = raster_info.xmin + col_end * blocksize * raster_info.pixel_width
            win_ymax = (
                raster_info.ymax + row_start * blocksize * raster_info.pixel_height
            )  # pixel_height negative
    ...
    final_layer = final_ds.CreateLayer(
        "merged_polygons", srs=raster_info.projection, geom_type=ogr.wkbMultiPolygon
    )
    ...
    task4 = progress.add_task(
        "Writing merged polygons to disk...", total=len(components)
    )
    for root, members in components.items():
        ...
        if len(members) == 1:
            ...
            block_ids = sorted(
                set(blkid_store[fid] for fid in members if blkid_store[fid] is not None)
            )
